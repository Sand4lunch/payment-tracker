// Payment Tracker App - Enhanced with Advanced Filtering
// =========================================
...
class PaymentTrackerApp {
    constructor() {
        ...
        this.currentProject = '';
        this.selectedProjects = []; // Multi-project selection
        this.selectedYears = []; // Year filter
        this.currentSort = 'dueDate'; // Default sorting
    ...
    // ENHANCED: Get filtered payments with all filters applied
    getFilteredPayments() {
    ...
    // ENHANCED: Apply sorting to filtered payments
    applySorting(payments) {
        ...
        switch(this.currentSort) {
            ...
            case 'dueDateDesc':
                ...
                break;
            case 'amountAsc':
                sorted.sort((a, b) => (a.amountOwed || 0) - (b.amountOwed || 0));
                break;
            case 'amountDesc':
                sorted.sort((a, b) => (b.amountOwed || 0) - (a.amountOwed || 0));
                break;
            case 'project':
    ...
    // ENHANCED: Populate project filter with checkboxes
    ...
    populateYearFilter() {
        ...
        const years = [...new Set(
            this.payments
                .filter(p => p.dueDate)
                .map(p => new Date(p.dueDate).getFullYear().toString())
        )].sort().reverse();
        ...
        // Add event listeners
        container.querySelectorAll('.year-checkbox').forEach(checkbox => {
            checkbox.addEventListener('change', (e) => {
    ...
    // ENHANCED: Render filtered totals
    renderFilteredTotals() {
        const container = document.getElementById('filteredTotals');
        if (!container) return;
        ...
        container.innerHTML = `
            <div class="filtered-stats">
                <div class="filtered-stat">
                    <span class="filtered-stat-label">Showing:</span>
                    <span class="filtered-stat-value">${filtered.length} payments</span>
                </div>
                <div class="filtered-stat">
                    <span class="filtered-stat-label">Total Owed:</span>
                    <span class="filtered-stat-value amount">${this.formatCurrency(totalOwed)}</span>
                </div>
                <div class="filtered-stat">
                    <span class="filtered-stat-label">Total Paid:</span>
                    <span class="filtered-stat-value">${this.formatCurrency(totalPaid)}</span>
                </div>
                ${lateCount > 0 ? `
                <div class="filtered-stat">
                    <span class="filtered-stat-label">Late:</span>
                    <span class="filtered-stat-value" style="color: var(--danger);">${lateCount}</span>
                </div>
                ` : ''}
            </div>
        `;
    ...
    // ENHANCED: Render payments with all new features
    renderPayments() {
        ...
        let filtered = this.getFilteredPayments();
